...
const LOGIN_ERROR_RESPONSES = {
  ...
  WRONG_ORGANIZATION: [401, 'This account is not registered with the selected organization', 'Unauthorized'],
  ORGANIZATION_INACTIVE: [403, 'This organization is not currently active', 'Forbidden'],
};
...
function login(req, res) {
  ...
  if (result.error) {
    const [status, message, error] = LOGIN_ERROR_RESPONSES[result.error] || LOGIN_ERROR_RESPONSES.INVALID_CREDENTIALS;
    logger.log(`❌ LOGIN FAILED  email=${email}  reason=${result.error}`);
    return res.status(status).json({ message, error, statusCode: status });
  }
  logger.log(`✅ LOGIN  role=${result.role}  email=${email}  org=${result.tenant ? result.tenant.organization_id : '?'}`);
  res.status(200).json(result);
}

const SIGNUP_ERROR_RESPONSES = {
  EMAIL_TAKEN: [409, 'Email already registered', 'Conflict'],
  INVALID_ORGANIZATION: [400, 'Selected organization is not available', 'Bad Request'],
};

function signup(req, res) {
  const result = authService.signup(req.body);
  if (result.error) {
    const [status, message, error] = SIGNUP_ERROR_RESPONSES[result.error] || SIGNUP_ERROR_RESPONSES.INVALID_ORGANIZATION;
    return res.status(status).json({ message, error, statusCode: status });
  }
