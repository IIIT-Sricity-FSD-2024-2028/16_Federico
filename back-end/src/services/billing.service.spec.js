...
describe('services/billing.service', () => {
  ...
  it('addLedgerEntry() mints a PER-LEDGER entry_id sequence, not a global max (documented intentional behavior)', () => {
    ...
    const entryA = billingService.addLedgerEntry({
    ...
    const entryB = billingService.addLedgerEntry({
      ledger_id: ledger.ledger_id,
      service_id: 2,
      quantity: 1,
      unit_price: 200,
    ...
    const ledger2 = billingService.createLedger({
      admission_id: admission2.admission_id,
      status: 'OPEN',
      organization_id: 1,
      hospital_id: 1,
    });
    const entryC = billingService.addLedgerEntry({
  ...
  it('createPayment() marks the ledger PAID, the admission PAYMENT_CONFIRMED, and auto-generates a receipt carrying the tenant stamp', () => {
    ...
    expect(
      dataStore.ledgers.find((l) => l.ledger_id === ledger.ledger_id).status,
    ).toBe('PAID');
    expect(
      dataStore.admissions.find(
        (a) => a.admission_id === admission.admission_id,
      ).status,
    ).toBe('PAYMENT_CONFIRMED');
    expect(dataStore.receipts.length).toBe(receiptCountBefore + 1);

    const receipt = dataStore.receipts.find(
  ...
  it('findPatientBills() pairs every admission with its ledger and entries, even when a ledger does not exist yet', () => {
    ...
    billingService.createLedger({
      admission_id: admissionWithLedger.admission_id,
      status: 'OPEN',
      organization_id: 1,
      hospital_id: 1,
    });
    ...
    const withLedgerEntry = bills.find(
      (b) => b.admission.admission_id === admissionWithLedger.admission_id,
    );
    const withoutLedgerEntry = bills.find(
      (b) => b.admission.admission_id === admissionWithoutLedger.admission_id,
    );
