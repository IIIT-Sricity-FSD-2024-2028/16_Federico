...
describe('services/organization.service', () => {
  ...
  it('setStatus()/remove() transition status without deleting the record (soft delete)', () => {
    ...
    expect(organizationService.findById(org.organization_id).status).toBe('SUSPENDED');
  ...
  it('createHospital() marks the first branch primary automatically', () => {
    ...
    const first = organizationService.createHospital(org.organization_id, { name: 'Main Campus' });
    const second = organizationService.createHospital(org.organization_id, { name: 'Second Branch' });

    expect(first.is_primary).toBe(true);
    expect(second.is_primary).toBe(false);
    expect(organizationService.primaryHospitalFor(org.organization_id).hospital_id).toBe(first.hospital_id);
    expect(organizationService.hospitalsFor(org.organization_id)).toHaveLength(2);
  });
  ...
  it('setModuleFlags()/enabledModulesFor() round-trip and default every unconfigured module to disabled', () => {
    ...
    organizationService.setModuleFlags(org.organization_id, ['BILLING', 'APPOINTMENTS']);
    ...
    expect(allFlags.find((f) => f.module_code === 'INSURANCE').enabled).toBe(false);
  });
  ...
  it('marketplaceListing() only includes ACTIVE organizations and never leaks internal fields', () => {
    ...
    const suspended = organizationService.create({ name: 'Unit Test Hospital F' });
  ...
  it('usageFor() counts records scoped to the organization only', () => {
    ...
    organizationService.createHospital(orgA.organization_id, { name: 'A Campus' });
    organizationService.createHospital(orgB.organization_id, { name: 'B Campus' });
