...
describe('services/admission.service', () => {
  it('create() defaults admit_time to now when not supplied, but respects an explicit one', () => {
    const withoutTime = admissionService.create({
      patient_id: 201,
      bed_id: 1,
      status: 'ADMITTED',
      organization_id: 1,
    ...
    const explicitTime = admissionService.create({
      patient_id: 201,
      bed_id: 1,
      status: 'ADMITTED',
      admit_time: '2026-01-01T00:00:00.000Z',
      organization_id: 1,
      hospital_id: 1,
    });
  ...
  it('findOne()/update() round-trip and return null for an unknown id', () => {
    ...
    const updated = admissionService.update(admission.admission_id, {
      status: 'DISCHARGED',
    });
