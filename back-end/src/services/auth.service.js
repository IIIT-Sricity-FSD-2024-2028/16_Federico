...
function tenantContextFor(user) {
  ...
  const enabledModules = organizationService.enabledModulesFor(organization.organization_id);
  return {
...
function login(email, password, requestedOrganizationId) {
  ...
  if (requestedOrganizationId && user.organization_id !== requestedOrganizationId) {
  ...
  activityService.log('info', `${role} login: ${user.name}`, { userId: user.user_id }, user.organization_id);
...
function signup(payload) {
  ...
  const primaryHospital = organizationService.primaryHospitalFor(organization.organization_id);
  ...
  activityService.log('success', `New patient registered: ${patient.name} (${patient.uhid})`, { patientId: patient.patient_id }, organization.organization_id);

  return { token, role: 'Patient', user: toPublicUser(newUser), patient, tenant: tenantContextFor(newUser) };
}
...
function me(session) {
  ...
  return { role: session.role, user: toPublicUser(user), patient, tenant: tenantContextFor(user) };
}
...
module.exports = { login, signup, me, logout, findUserByEmail, findPatientForUser, roleNameFor, tenantContextFor };
