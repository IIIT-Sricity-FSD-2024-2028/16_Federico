...
// Log buffer for regular interval flushing
const logBuffer = {
  access: [],
  error: [],
  combined: [],
...
if (flushTimer.unref) {
  flushTimer.unref(); // Don't block Node process exit in tests
}

// Ensure logs are flushed on process termination
...
function logAccess(req, res, durationMs) {
  ...
  const line = formatLogEntry('HTTP', `${req.method} ${req.originalUrl || req.url} ${res.statusCode} (${durationMs}ms)`, meta);
  logBuffer.access.push(line);
  logBuffer.combined.push(line);

  // If buffer exceeds 50 lines, flush immediately
...
function logError(err, req = null) {
  const meta = {
    ...
    tenantId: req?.tenant?.organizationId || null,
    userId: req?.session?.userId || null,
    body: req && req.body && Object.keys(req.body).length > 0 ? req.body : undefined,
  };
  ...
  // Flush errors immediately to disk
  flushLogs();
}
...
function getLogFilesInfo() {
  ...
  const getInfo = (filePath) => {
    ...
    return { sizeBytes: stat.size, modified: stat.mtime.toISOString(), exists: true };
  };
