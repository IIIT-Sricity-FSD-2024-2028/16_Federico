...
document.addEventListener("DOMContentLoaded", function () {
  ...
  (async function loadAvailablePlans() {
    try {
      var plans = await window.ApiClient.marketplace.plans();
        ...
        if (container) {
          container.innerHTML = plans.map(function (p) {
            ...
            var modulesList = (p.included_modules || []).map(function (m) {
              return '<li>' + m.toLowerCase().replace(/_/g, ' ') + '</li>';
            }).join("");

            return (
              '<div class="plan-select-card ' + isSelected + '" data-plan-id="' + p.plan_id + '" data-price="' + p.price_monthly + '" data-name="' + p.name + '">' +
              isPopular +
              ...
              '<li>Up to ' + p.max_beds + ' Beds Quota</li>' +
              '<li>' + p.max_users + ' Staff Users</li>' +
              '<li>' + p.max_hospitals + ' Hospital Campus(es)</li>' +
      ...
      // Keep pre-rendered HTML cards as fallback
      attachPlanCardListeners();
    }
  })();
  ...
  // ---- Form Submission (Checkout & Automatic Tenant Provisioning) ----
  ...
  onboardingForm?.addEventListener("submit", async function (e) {
    ...
    try {
      var response = await window.ApiClient.marketplace.registerOrganization(payload);
      ...
      metaBox.innerHTML =
        ...
        '<div><strong>Tenant Identifier:</strong> <code style="background:var(--md-surface-container);padding:2px 6px;border-radius:4px;">tenant_' + provisioned.organization.organization_id + '</code></div>' +
        '<div><strong>Primary Campus Branch:</strong> ' + provisioned.hospital.name + '</div>' +
        '<div><strong>Active Plan:</strong> ' + selectedPlan.name + ' Tier (₹' + selectedPlan.price.toLocaleString("en-IN") + '/mo)</div>' +
        '<div><strong>Admin Account:</strong> ' + provisioned.admin.email + '</div>' +
        '<div><strong>Live API Gateway Key:</strong> <code style="background:var(--md-surface-container);padding:2px 6px;border-radius:4px;">' + (provisioned.apiKey ? provisioned.apiKey.key : "fed_live_...") + '</code></div>' +
        '</div>' +
        ...
        'Enabled Services: ' + modules.map(function(m){ return '<strong>' + m + '</strong>'; }).join(', ') +
        '</div>';
      ...
      document.getElementById("btn-launch-admin").addEventListener("click", function () {
        // Redirect to Hospital Admin Portal
